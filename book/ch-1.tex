...
Another source of privacy leaks can be certain kinds of usability
flaws in software that people use. These flaws can make it appear that
information has been removed or destroyed, when in fact the
information is only hidden.
...
\sgraphic[width=3in]{ch-1/ms-tool}{\caption{Microsoft Word 2010's
...
(Figure~\ref{demo1}), and both can be independently recovered.
...
\sgraphic{ch-1/fig2}{The Text Highlight tool places a colored box underneath the
  ...
  be absent, but it is still present in the resulting PDF file.}\label{demo1}
\end{figure}
...
Technical privacy auditing is an approach of looking for private
