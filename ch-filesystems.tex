...
recover files depends not just on the specific operating system that
...
bytes, also called \emph{sectors}. These blocks are traditionally
sized as a power of 2. In the 1990s most hard drives had a block
size of 512~B; modern drives (both SSD and HD) have a block size of
4~KiB.\footnote{Compact Discs (CDs) have a block size of 2048~B.} (For an explanation as to why blocks are sized in powers of
...
\twofigures{.55\textwidth}{ch-fs/1024px-Seagate_ST33232A_hard_disk_inner_view.jpg}{Inner
  view of a Seagate 3.5 inches hard disk drive Medalist ST33232A model
  manufactured in Malaysia in 1998. This drive has 3 platters, 6
  physical read/write heads, and a total storage of 3,227~MB. Eric
  Gaba, Wikimedia Commons}{.4\textwidth}{ch-fs/sdcard}{Two SanDisk SD
  Cards. Each drive has 1,000~MB of solid state storage. Simson L.\ Garfinkel}
...
in the 1980s for use with the original IBM Personal Computer. FAT is
...
digital cameras.\footnote{There are actually more than a dozen
...
FAT divides the disk into four regions:
\begin{itemize}
...
\item \emph{The data region}, which holds \emph{data
  ...
  directory contents. Each cluster consists of 1, 2, 4, 8 or more
  sequential 512-byte sectors. 
\end{itemize}

This is shown graphically in \figref{fig:fat16}

\begin{figure}
\begin{tabular}{|l|l|l|l|l|}
\hline
        &             &                     & \multicolumn{2}{c|}{sectors 63--250,781}\\
sector 9 & sectors 1--31 & sectors 32--62   & \multicolumn{2}{c|}{Data Area}\\
\cline{4-5}
BPB     & FAT \#0     & FAT \#1        & sectors 63--94  & sectors 95--250,782\\
        &             &                & root directory  & cluster area    \\
\hline
\end{tabular}    
\caption{A sample FAT16 file system from a 128MB SD camera card. This
  FAT16 drive has a cluster size of 16,384.\label{fig:fat16}}
\end{figure}
...
an array of 12, 16 or 32-bit integers, with each slot in the array
corresponding to a specific data cluster. 
...
producing a so-called ``8.3 filename.'' The characters were limited to
upper-case ASCII letters, numbers, and a few symbols. The major
innovation of FAT32 was a technique that Microsoft developed that
allowed long file names to be represented in a manner that was
invisible to legacy systems that still ran the old DOS systems. As a
result, every file on a FAT32 file system can have two names---a long
name with mixed case, and a shorter 8.3 name with only uppercase
letters. Today most FAT implementations will allow long file names even on media
formatted as FAT12 or FAT16.

\tabref{table:fatdir} shows the major elements of the FAT directory
entry.
...
|00h| & |1| & |00h| -- This is the last entry, and it is available \\
|00h| & |1| & |E5h| -- This entry is deleted; ignore it \\
\hline
...
|14h| & |2| & FAT32: Top 16 bits of cluster with start of file\\
|16h| & |2| & last write time\\
|18h| & |2| & last write date\\
|1Ah| & |2| & Bottom 16 bits of start cluster\\
|1Ch| & |4| & Size of file\\
\end{tabular}
\caption{The 32-bytes of the FAT directory entry.\label{table:fatdir}}
\end{table}

The definitive specification of the Microsoft's various
implementations of the FAT file system is
  the \emph{Microsoft Extensible Firmware Initiative FAT32 File System
    Specification}, rev. 1.03, Dec 6,
  2000\cite{microsoft-efi}. However, for a more complete description,
  see the Wikipedia entry, as it covers many different implementations
...
For example, a 128~MB drive formatted with a cluster size of 16~KiB
would have roughly 7800 clusters. Because each file must start on a
cluster boundary, such a file system could support a maximum of 7800
individual files.

Thus, a 128~MB drive formatted with FAT16 Values stored can indicate
that the cluster is free for use or that it is currently being used by
a file. If the cluster is used by a file, the slot in the array
contains the cluster number of next cluster in the file. The array
thus contains a linked list for each file on the disk of all the
clusters associated with each file.

Finally, the computer reads the root directory into memory. The
directory is a set of records where each record contains the file's
name, it's length, and the cluster number where the file's contents
start.
...
formatted. Therefore, a FAT32 file system with a cluster size of 1 has
a maximum file system size of $512\times2{32}=2\textrm{GiB}$, while a
cluster size of 8 allows for a file system up to 16~GiB. 

Each FAT file has a number which is the number of the cluster where
the file's content starts. The array conveys, for each cluster,
whether the cluster is currently free or allocated to a file. The FAT itself is a linked list that
denotes, for each cluster, 

Every
file starts on a cluster boundary, and each cluster can be used by
...
the IBM Personal Computer and used with minor revisions ever since.
...
An excellent source for information about the various MBR formats is
the Wikipedia
page\footnote{\url{http://en.wikipedia.org/wiki/Master_boot_record}}. If
you review the page you will see that no less than six different MBRs
have been developed and deployed since MBR records were introduced by
IBM PC DOS 2.0 in 1983. All of these systems store the bootstrap code
at location 0, the first four partition entries starting at location
...
\begin{tabular}{|l|l|l|}
\hline
block 0: & blocks 1--50 & blocks 51--60799         \\
MBR      & slack space  & Partition \#2 DOS FAT16  \\
\hline
\end{tabular}
