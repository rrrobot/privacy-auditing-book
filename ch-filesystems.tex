...
\begin{description}
\item[FAT (File Allocation Table)] A simple file system developed by
  Microsoft in the 1980s for use with the original IBM Personal
  Computer. Today FAT is widely used in digital cameras and cell
  phones, but it is rarely used on personal computers.
...
4096 bytes.\footnote{Compact Discs (CDs) have a block size of 2048~B,
  although some CD drives have a switch which allows the firmware to
  simulate a read-only hard drive that has a block size of 512~B for
  backwards compatibility.}
A 1~TB drive therefore has approximately 250 million 4096-byte
blocks. 
...
\lstinputlisting[caption=A simple program to clear a disk.]{ch-fs/clear.py}
\begin{figure}
\begin{lstlisting}
\end{lstlisting}
...
* Creating files with self-identifying names and content.
